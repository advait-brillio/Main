...
server.listen(process.env.port || process.env.PORT || 6190, function () {
...
function login(session) {
    ...
    session.send(msg);
    builder.Prompts.text(session, "Please type 'login' once you see the welcome message in the browser.");
}
    ...
    if (error) {
        ...
        getUserEmail(token.token.access_token, function (error, email) {
            ...
            if (error) {
                ...
                response.writeHead(302, { 'Location': 'http://localhost:3000/code' });
